...
complete -c tmux-im -n __fish_use_subcommand -a clear-status -d 'Clear IM status for a pane'

complete -c tmux-im -n '__fish_seen_subcommand_from save on clear-status' -a '(tmux list-panes -a -F "#{pane_id}" 2>/dev/null)' -d 'Pane ID'
