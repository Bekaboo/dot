...
---@param ntype string|string[]|fun(type: string): boolean type of node, or function to check node type
...
function M.in_node(ntype, opts)
  ...
  for _, t in ipairs(ntype --[=[@as string[]]=]) do
    if node:type():match(t) then
      return true
    end
  end
