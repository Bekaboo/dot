  ...
  vim.system({
    ...
    vim.system(
      ...
      vim.schedule_wrap(function(obj2)
        ...
        vim.notify(
          '[my.plugin.jupytext] error writing to notebook: ' .. obj2.stderr,
...
local function jupytext_convert(buf)
  ...
  if not vim.uv.fs_stat(cache_dir) and not vim.uv.fs_mkdir(cache_dir, 511) then
    vim.notify(
      '[my.plugin.jupytext] cannot create cache dir ' .. cache_dir,
      vim.log.levels.ERROR
    )
    return
  end
  ...
  if not fs.is_empty(ipynb) and not vim.uv.fs_stat(md) then
    ...
    if obj.code == 0 then
      ...
      vim.schedule(function()
        vim.notify(
          '[my.plugin.jupytext] error converting notebook: ' .. obj.stderr,
          vim.log.levels.ERROR
        )
      end)
    end
