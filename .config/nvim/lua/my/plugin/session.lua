...
M.opts = {
  ...
  ---@return string?
  root = function(path)
    return require('my.utils.fs').root(path)
...
function M.load(session, notify)
  ...
  if has_valid_buf() then
    local response = vim.fn.confirm(
      string.format(
        "[my.plugin.session] non-empty buffers exist, confirm loading new session from '%s'?",
