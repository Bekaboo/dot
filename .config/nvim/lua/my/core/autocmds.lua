...
do
  ...
  augroup('my.bigfile', {
    ...
    { 'TextChanged', 'FileType' },
    {
      ...
      callback = function(args)
        local buf = args.buf
        if vim.b[buf].bigfile and require('my.utils.ts').is_active(buf) then
  ...
  augroup('my.auto_cwd', {
    'BufEnter',
    {
      ...
      callback = function(args)
        ...
        local fs_utils = require('my.utils.fs')
        local root_dir =
  ...
  augroup('my.keep_win_ratio', {
    { 'VimResized', 'TabEnter' },
    {
      ...
      callback = function()
        ...
        require('my.utils.win').restore_ratio(win_ratio)
      end,
    },
  }, {
    'WinResized',
    {
      ...
      callback = function()
        ...
        require('my.utils.win').save_ratio(win_ratio, vim.v.event.windows)
      end,
    },
  }, {
    { 'TermOpen', 'WinNew' },
    {
      desc = 'Record window ratio.',
      callback = function()
        require('my.utils.win').save_ratio(win_ratio, vim.api.nvim_list_wins())
      end,
    },
  })
end
...
do
  ...
  local function win_save_fixed_heights()
    require('my.utils.win').save_heights(
      win_heights,
      vim
  ...
  augroup('my.fix_winfixheight_with_winbar', {
    ...
    'WinResized',
    {
      ...
      callback = function()
        ...
        require('my.utils.win').restore_heights(win_heights)
      end,
    },
  }, {
    'FileType',
    ...
    'SessionLoadPost',
    {
      ...
      callback = function()
        ...
        for _, buf in ipairs(vim.api.nvim_list_bufs()) do
          if whitelist[buf] or not require('my.utils.buf').is_empty(buf) then
            goto continue
          end
