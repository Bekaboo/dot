...
do
  ...
  require('my.utils.load').on_events('BufReadPre', 'my.opt.shada', rshada)
  ...
  require('my.utils.load').on_events(
    ...
    vim.schedule_wrap(function()
      ...
      for _, win in ipairs(vim.api.nvim_list_wins()) do
        if require('my.utils.opt').spell:was_locally_set({ win = win }) then
          goto continue
        end
      ...
      for buf, _ in pairs(bufs) do
        if require('my.utils.ts').is_active(buf) then
...
vim.opt.quickfixtextfunc = [[v:lua.require'my.utils.opts'.qftf]]
