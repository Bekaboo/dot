...
---@class (partial) my.lsp.config : vim.lsp.Config
...
function M.start(config, opts)
  ...
  local function validate(dir)
    ...
    return dir ~= nil
        ...
        and not require('my.utils.fs').is_home_dir(dir)
        and not require('my.utils.fs').is_root_dir(dir)
        and dir
      or nil
  end

  local root_dir = validate(
    require('my.utils.fs').root(
      bufname,
      vim.list_extend(
...
---@param opts my.lsp.soft_stop.opts?
...
---@param range my.lsp.range 0-based range
